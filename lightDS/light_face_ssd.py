...
from lightDS.data import widerface_640
from lightDS.layers import *
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, num_classes):
        ...
        if phase == 'test':
            ...
            self.detect = Detect(num_classes, 0, cfg['num_thresh'], cfg['conf_thresh'],
                                 cfg['nms_thresh'])
    ...
    def forward(self, x):
        ...
        if pa and self.phase == "train":
            ...
            mbox_num = cfg['mbox'][0]
            head_loc = torch.cat(
                [o[:, :, :, :4 * mbox_num].contiguous().view(o.size(0), -1) for o in pa_loc], 1)
            head_conf = torch.cat(
                [o[:, :, :, :2 * mbox_num].contiguous().view(o.size(0), -1) for o in pa_conf], 1)
            body_loc = torch.cat(
                [o[:, :, :, 4 * mbox_num:8 * mbox_num].contiguous().view(o.size(0), -1) for o in
                 pa_loc[1:]], 1)
            body_conf = torch.cat(
                [o[:, :, :, 2 * mbox_num:4 * mbox_num].contiguous().view(o.size(0), -1) for o in
                 pa_conf[1:]], 1)
        ...
        if self.phase == "test":
            ...
            if pa:
                ...
                self.head_priors = self.init_priors(self.cfg, min_size=cfg['min_sizes'][:-1],
                                                    max_size=cfg['max_sizes'][:-1])
                self.body_priors = self.init_priors(self.cfg, min_size=cfg['min_sizes'][:-2],
                                                    max_size=cfg['max_sizes'][:-2])
                output = (
            ...
            if refine:
                output = output + tuple((arm_loc.view(arm_loc.size(0), -1, 4),
                                         arm_conf.view(arm_conf.size(0), -1, self.num_classes)))

        if self.phase == "test":
            print(
                'Backbone: %.4f , FPN: %.4f, CPM: %.4f, Head: %.4f, PA: %.4f, Decode_NMS: %.4f' % (
...
def multibox(input_channels, mbox_cfg, num_classes):
    ...
    for k, v in enumerate(input_channels):
        loc_layers += [
            nn.Conv2d(input_channels[k], mbox_cfg[k] * 4, kernel_size=3, dilation=1, stride=1,
                      padding=1)]
        conf_layers += [
            nn.Conv2d(input_channels[k], mbox_cfg[k] * num_classes, kernel_size=3, dilation=1,
                      stride=1, padding=1)]
...
def arm_multibox(output_channels, mbox_cfg, num_classes):
    ...
    for k, v in enumerate(output_channels):
        ...
        conf_layers += [
            nn.Conv2d(input_channels, mbox_cfg[k] * num_classes, kernel_size=3, padding=1)]
    return loc_layers, conf_layers
...
class DeepHeadModule(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.conv1 = BasicConv2d(self._input_channels, self._mid_channels, kernel_size=1,
                                 dilation=1, stride=1, padding=0)
        self.conv2 = BasicConv2d(self._mid_channels, self._mid_channels, kernel_size=3, dilation=1,
                                 stride=1, padding=1)
        self.conv3 = nn.Conv2d(self._mid_channels, self._output_channels, kernel_size=1, dilation=1,
                               stride=1, padding=0)
