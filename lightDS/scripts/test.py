...
def vis_detections(im,  dets, image_name , thresh=0.5):
    ...
    for i in inds:
        ...
        cv2.imwrite('img_cut/huong_design/'+str(random.randint(1,100000000))+'.png',im[int(bbox[1]):int(bbox[3]),int(bbox[0]):int(bbox[2])])
...
def light_test_oneimage(frame):
    img_id = 'result'
    shrink = 1
    det = infer(net , frame , transform , thresh , cuda , shrink)
    im = vis_detections(frame , det , img_id, 0.6)
    return im
...
if __name__ == "__main__" :
    ...
    cap = cv2.VideoCapture("video/huong_design.avi")
    hasFrame, frame = cap.read()
    while(1):
        hasFrame, frame = cap.read()
        if not hasFrame:
            break
        frame = frame[173:580, 180:540]
        im = light_test_oneimage(frame)

        cv2.imshow("Face Detection Comparison", im)
